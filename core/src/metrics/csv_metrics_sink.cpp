...
public:
    ...
    void on_run_info(const RunInfo & r) override {
        ...
        std::fprintf(f_,
                     ...
                     "flash_attention=%s rope_scaling=%s rope_freq_base=%.8g rope_freq_scale=%.8g "
                     "yarn_ext_factor=%.8g yarn_attn_factor=%.8g yarn_beta_fast=%.8g yarn_beta_slow=%.8g "
                     "yarn_orig_ctx=%d custom_chat_template=%d\n",
                     ...
                     r.flash_attention.c_str(), r.rope_scaling.c_str(), (double) r.rope_freq_base,
                     (double) r.rope_freq_scale, (double) r.yarn_ext_factor, (double) r.yarn_attn_factor,
                     (double) r.yarn_beta_fast, (double) r.yarn_beta_slow, r.yarn_orig_ctx, r.custom_chat_template);
        std::fprintf(f_,
