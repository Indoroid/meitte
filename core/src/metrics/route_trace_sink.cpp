...
public:
    ...
    void on_static(const RouteTraceStatic & s) override {
        ...
        std::fprintf(f_, "turn,phase,step,layer,slot,expert,weight,residency,expert_bytes,dropped,media_kind\n");
        std::fflush(f_);
    }
    ...
    void on_rows(const RouteTraceRow * rows, size_t n) override {
        for (size_t i = 0; i < n; ++i) {
            ...
            if (std::isnan(r.weight))
                std::fprintf(f_, "%d,%d,%d,%d,%d,%d,nan,%u,%llu,%u,%u\n", r.turn, r.phase, r.step, r.layer, r.slot,
                             (int) r.expert, (unsigned) r.residency, (unsigned long long) r.expert_bytes,
                             (unsigned) r.dropped, (unsigned) r.media_kind);
            else
                std::fprintf(f_, "%d,%d,%d,%d,%d,%d,%.6g,%u,%llu,%u,%u\n", r.turn, r.phase, r.step, r.layer, r.slot,
                             (int) r.expert, (double) r.weight, (unsigned) r.residency,
                             (unsigned long long) r.expert_bytes, (unsigned) r.dropped, (unsigned) r.media_kind);
        }
