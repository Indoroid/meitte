...
- **Full server/CLI option parity.** Every model, sampling, streaming, cache, speculation, prediction,
  and diagnostic option applicable to a persistent server is now parsed and wired, including CSV,
  route, compute, layer-compute and I/O trace sinks. A parity gate compares both parsers so future
  CLI flags cannot silently disappear from the server. `--session` is an accepted no-op because the
  server is always persistent; `--progress` retains its stdout telemetry alongside HTTP/SSE.
- **OpenAI tool conversations.** Chat requests now preserve tool definitions, assistant tool calls,
...
- **`bmoe-server` preserves complete chat requests.** System messages and earlier user/assistant
  turns now reach the model template instead of reducing every request to its last user message.
- **`bmoe-server` honors request sampling.** `temperature` and `top_p` now rebuild the lightweight
  sampler chain per request while the model and expert cache stay resident.
- **`bmoe-server` completion streaming uses the OpenAI shape.** Legacy `/v1/completions` SSE chunks
  emit `choices[].text`; chat chunks continue to emit `choices[].delta.content`.
- **`bmoe-server` applies ChatML by endpoint.** `/v1/chat/completions` always renders the complete
  messages array through the model's own Jinja template. `/v1/completions` remains a raw prompt API
  unless `--chatml` is supplied, matching `bmoe-cli` exactly while one loaded session serves both.
