...
`--predict-log`, speculation, expert streaming, cache policy, and deprecated dense-weight aliases.
...
The live contract suite uses Liquid AI's official LFM2.5-8B-A1B Q4_K_M GGUF and covers model
discovery, raw completion, ChatML, reasoning separation, ordinary SSE, OpenAI tool calls, tool-result
round trips, tool-call SSE, metrics CSV, and the serial expert-streaming path:

```bash
python3 scripts/test-lfm25-server.py \
  --server build/cli/bmoe-server \
